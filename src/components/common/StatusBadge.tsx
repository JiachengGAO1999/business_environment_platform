...
const statusConfig: Record<ReportStatus, { label: string; className: string }> = {
  not_started: { label: '未开始', className: 'bg-gray-100 text-gray-600 border-gray-300' },
  ...
  archived: { label: '已归档', className: 'bg-gray-100 text-gray-500 border-gray-300' },
}
...
export function ReviewStatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; className: string }> = {
    pending: { label: '待审核', className: 'bg-gray-100 text-gray-600 border-gray-300' },
    ...
    delete: { label: '已删除', className: 'bg-gray-100 text-gray-500 border-gray-300' },
  }
